...
The app is plain HTML, CSS and JavaScript — no build step, no server. Any static
host works. For GitHub Pages:

1. Push this folder to a repository.
2. Copy `pages-workflow.yml` to `.github/workflows/pages.yml`.
3. In **Settings → Pages**, set Source to **Deploy from a branch**, branch
   **`gh-pages`**, folder **`/ (root)`**.
4. Every push then publishes to `https://<owner>.github.io/<repo>/`.

---

## Things worth knowing

- **No logins.** Everyone shares one key, so anyone with the app URL can read and
  write. That's deliberate — no passwords for the crew to lose. Don't put anything
  confidential in it and don't hand the URL out beyond RCK.
...
| `pages-workflow.yml` | Copy to `.github/workflows/pages.yml` to auto-deploy |
